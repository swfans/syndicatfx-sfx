...
Sound samples source for Syndicate Fan Expansion.
...
Download the release `syndicatfx-sfx-*.zip` file. Copy the extracted
`SOUND` folder to a previous SyndicatFX installation,
replacing the existing files.

### General building instructions

To build **SyndicatFX SFX**, use the following steps:

1. download [sndbanker](https://github.com/swfans/sndbanker/releases/),
   place the binary in `syndicatfx-sfx/bin` or any folder included in `PATH` env var.
2. go into the directory with `syndicatfx-sfx` source release
...
Download the `syndicatfx-sfx` source release, and extract it somewhere.

Also download latest release of the [sndbanker](https://github.com/swfans/sndbanker/releases/),
and extract the binary into `bin` sub-folder of the place with `syndicatfx-sfx`.
...
Being still in the `syndicatfx-sfx` folder, generate build scripts from templates using
autotools:
